...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.lineEdit.move(360,50)

        ## UI 버튼 (단일 정보 얻기)
        btnGetInformationSingle = QPushButton("단일 정보 얻기", self)
        btnGetInformationSingle.move(480, 50)
        ...
        btnGetInformationAll.move(480, 90)
        btnGetInformationAll.clicked.connect(self.btnGetInformationAll_clicked)

        ## UI 버튼 (정보 얻기)
        btnGetInformation = QPushButton("주봉차트정보", self)
        btnGetInformation.move(480, 130)
        btnGetInformation.clicked.connect(self.btnGetInformationWeek_clicked)

        ## UI 버튼 (정보 얻기)
        btnGetInformation = QPushButton("일봉차트정보", self)
...
#            for i in information:
        ...
        elif rqname == 'opt10001_req_single':
            name = self.kiwoom.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", rqname, 0, "종목명")
            code = self.kiwoom.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", rqname, 0, "종목코드")
            volume = self.kiwoom.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", rqname, 0, "거래량")
            currentPrice = self.kiwoom.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", rqname, 0, "현재가")
            ...
            information = [code.strip(), name.strip(), volume.strip(), currentPrice.strip(), PER.strip(), EPS.strip(), ROE.strip(), PBR.strip(), EV.strip(), BPS.strip(), salesAccount.strip(), operatingProfit.strip(), perProfit.strip(), total.strip()]
            print(information)
            self.listWidget.addItems(information)
            ...
            for x in temp :
                CSVFileIO.setCsv("E:\99.Coding\\18_D_Apple\Data\\csv\\daydata_"+ code + ".csv", x)
            print("종목코드 : " + code + " : 주식일봉차트데이터 저장")
    ...
    def btnGetInformationSingle_clicked(self):
        ...
        name = self.kiwoom.dynamicCall("GetMasterCodeName(QString)", [x])
        self.kiwoom.dynamicCall("SetInputValue(QString, QString)", '종목코드', x)
        self.kiwoom.dynamicCall("CommRqData(QString, QString, int, QString)", 'opt10001_req_single', 'opt10001', '0', '0101')
